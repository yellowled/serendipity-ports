<!-- ENTRIES START -->
    ...
    <div class="post">
        <h2 class="title">{$entry.title}{if $dategroup.is_sticky} ({$CONST.STICKY_POSTINGS}){/if}</h2>

        <p class="byline"><small>{$CONST.POSTED_BY} <a href="{$entry.link_author}">{$entry.author}</a> {$CONST.ON} {$entry.timestamp|@formatTime:DATE_FORMAT_ENTRY}{if $entry.categories} {$CONST.IN} {foreach from=$entry.categories item="entry_category" name="categories"}<a href="{$entry_category.category_link}">{$entry_category.category_name|@escape}</a>{if not $smarty.foreach.categories.last}, {/if}{/foreach}{/if}{if $entry.is_entry_owner and not $is_preview}&nbsp;|&nbsp;<a href="{$entry.link_edit}">{$CONST.EDIT_ENTRY}</a>{/if}</small></p>
...
{if $entry.has_extended and not $is_single_entry and not $entry.is_extended}
            <a class="more" href="{$entry.link}#extended">{$CONST.VIEW_EXTENDED_ENTRY|@sprintf:$entry.title}</a>&nbsp;&nbsp;&nbsp;
{/if}
