<!-- ENTRIES START -->
    {assign var="mrtreadmore" value=$CONST.MRT_READMORE}
    ...
    {assign var="entry" value=$entry scope="parent"}
        <div class="post">
            <div class="title">
                <h2><a href="{$entry.link}">{$entry.title}</a>{if $dategroup.is_sticky} ({$CONST.STICKY_POSTINGS}){/if}</h2>
                <p><small>{$CONST.POSTED_BY} <a href="{$entry.link_author}">{$entry.author}</a> {$CONST.ON} {$entry.timestamp|@formatTime:'%H:%M'}</small></p>
            </div><!-- /.title -->
            ...
            <p class="links">{if $entry.has_extended and not $is_single_entry and not $entry.is_extended}<a class="more" href="{$entry.link}#extended">{$mrtreadmore}</a> &nbsp;&nbsp;&nbsp; {/if}{if $entry.has_comments}{if $use_popups}<a class="comments" href="{$entry.link_popup_comments}" onclick="window.open(this.href, 'comments', 'width=480,height=480,scrollbars=yes'); return false;">{if $entry.comments == 0}{$CONST.NO_COMMENTS}{else}{$entry.comments} {$entry.label_comments}{/if}</a>{else}<a class="comments" href="{$entry.link}#comments">{if $entry.comments == 0}{$CONST.NO_COMMENTS}{else}{$entry.comments} {$entry.label_comments}{/if}</a>{/if}{/if}</p>
