<!-- ENTRIES START -->
    ...
    {assign var="olto" value=$CONST.OL_TO}
    {foreach from=$entries item="dategroup"}
    {foreach from=$dategroup.entries item="entry"}
    {assign var="entry" value=$entry scope="parent"}
        <a id="{$entry.id}" href="{$entry.link}" rel="bookmark" title="Permanent Link: {$entry.title}"><h3>{$entry.title}{if $dategroup.is_sticky} ({$CONST.STICKY_POSTINGS}){/if}</h3></a>
    ...
    {/if}
        ...
        <div class="metadata">
            {$entry.timestamp|@formatTime:DATE_FORMAT_ENTRY}{if $entry.categories} | {foreach from=$entry.categories item="entry_category" name="categories"}<a href="{$entry_category.category_link}">{$entry_category.category_name|@escape}</a>{if not $smarty.foreach.categories.last}, {/if}{/foreach}{/if}{if $entry.has_comments}{if $use_popups} | <a href="{$entry.link_popup_comments}" onclick="window.open(this.href, 'comments', 'width=480,height=480,scrollbars=yes'); return false;">{$entry.label_comments} ({$entry.comments})</a>{else} | <a href="{$entry.link}#comments">{$entry.label_comments} ({$entry.comments})</a>{/if}{/if}{if $entry.has_trackbacks}{if $use_popups} | <a href="{$entry.link_popup_trackbacks}" onclick="window.open(this.href, 'comments', 'width=480,height=480,scrollbars=yes'); return false;">{$entry.label_trackbacks} ({$entry.trackbacks})</a>{else} | <a href="{$entry.link}#trackbacks">{$entry.label_trackbacks} ({$entry.trackbacks})</a>{/if}{/if}{if $entry.is_entry_owner and not $is_preview} | <a href="{$entry.link_edit}">{$CONST.EDIT_ENTRY}</a> {/if}
            {$entry.add_footer}
        </div>
        <!--
        ...
        {if $is_single_entry and not $use_popups and not $is_preview}
            ...
            <div class="serendipity_comments serendipity_section_trackbacks">
                <a id="trackbacks"></a>
                <h2 id="trackbacks">{$entry.trackbacks} {$entry.label_trackbacks} {$olto} &ldquo;{$entry.title}&rdquo;</h2>
        ...
        {if $is_single_entry and not $is_preview}
            <div class="serendipity_comments serendipity_section_comments">
                <a id="comments"></a>
                <h2 id="comments">{$entry.comments} {$entry.label_comments} {$olto} &ldquo;{$entry.title}&rdquo;</h2>
